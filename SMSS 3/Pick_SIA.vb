Public Class Pick_SIA
    ...
    Private Sub Back_Nav_Click(sender As Object, e As EventArgs) Handles Back_Nav.Click
        Pick_1.Show()                        'backwards navigation
        Me.Hide()
    End Sub
            ...
            If SSMS_DATABASEDataSet.Product_Bin.Rows(i).Item("BinID") = SelectedBin Then
                For j = 0 To Prowcount
                    If SSMS_DATABASEDataSet.Product_Bin.Rows(i).Item("ProductID") = SSMS_DATABASEDataSet.Product.Rows(j).Item("ProductID") Then
                        If SSMS_DATABASEDataSet.Product.Rows(j).Item("SIA") = "Yes" Then
                            ...
                            BindAmt = SSMS_DATABASEDataSet.Product_Bin.Rows(i).Item(3)                'calculates the stock replenishment list for the chosen bin and SIA flag
                            Cases = 0
                            If BindAmt > 0 Then
                            ...
                            If BinChoice.SelectedItem = SSMS_DATABASEDataSet.Product_Bin.Rows(j).Item("BinID") Then
                                ...
                                If BindAmt > 0 Then
                                    Do While SlfCnt + CasSiz <= MaxSlf
                                        SlfCnt = SlfCnt + CasSiz
                                        Cases = Cases + 1
                                        BindAmt = BindAmt - CasSiz                                              'calculates the exact amount of SIA stock that needs to be picked
                                        If BindAmt = 0 Then
                                            Exit Do
                                        End If
                                    Loop
